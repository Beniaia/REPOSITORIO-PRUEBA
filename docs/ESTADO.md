...
> Última actualización: **24 agosto 2026** · Fase actual: **F0 completa. F1 en marcha: flujo de reunión automática (Zoom + Calendar) con el código base escrito pero todavía no usable ni probado — falta la pantalla y la detección de respuesta. Pendiente también lanzar la primera tanda real del motor (10 leads de Levante)**
...
| Flujo de reunión automática (Zoom + Calendar) | 🟡 **Código base escrito el 24/08/2026, sin compilar ni probar, no usable todavía.** `lib/zoom.ts`, `lib/google-calendar.ts`, `lib/fecha-madrid.ts` y las acciones `registrarRespuestaLead`/`confirmarFechaReunion` en `lib/actions/leads.ts`. Falta: (1) el formulario en `app/(app)/leads/[id]/page.tsx` para poder usarlo desde la app, (2) `npm run build`/`typecheck` en verde, (3) una prueba real creando una reunión. Detección automática de respuesta (P-15) sigue sin construir — de momento sólo hay un registro manual |
...
9. **Mañana, en este orden — para que el flujo de reunión sea usable de verdad:**
   - a) Añadir el formulario en `app/(app)/leads/[id]/page.tsx`: registrar respuesta del lead (manual por ahora) + confirmar fecha → dispara `confirmarFechaReunion`.
   - b) `npm run build` y `npm run typecheck` en verde antes de dar nada por bueno (regla de `CLAUDE.md`, todavía no comprobado).
   - c) Probar de verdad: crear una reunión desde la app con el lead ficticio de prueba (`nuritina10@hotmail.com`) y comprobar que llegan el email, el Zoom y el evento de Calendar.
