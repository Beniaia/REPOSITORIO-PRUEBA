...
9. **Para que el flujo de reunión sea usable de verdad:**
   ...
   - c) **Probar de verdad — EMPEZADO Y SIN TERMINAR el 25/08/2026.** El bloqueo de login **no era de la app**: Nuria se había equivocado de contraseña; con la contraseña nueva (tras el reseteo por email) entra sin problema. **Sigue pendiente completar la prueba paso a paso desde la ficha del lead `nuritina10@hotmail.com`** (permiso de llamada → aprobar → Contactar → registrar respuesta → confirmar fecha) y comprobar que llegan el email, el Zoom y el evento de Calendar.
