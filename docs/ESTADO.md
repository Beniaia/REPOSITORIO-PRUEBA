...
| 24/08/2026 | **P-13 y P-14 resueltas.** App Zoom "Baladre Reuniones" (Server-to-Server OAuth) creada, activada (con el scope `meeting:write:meeting:admin`) y probada con un token real. Proyecto de Google Cloud `baladre-prospeccion-piloto` creado sin necesidad de la prueba gratuita de pago; Calendar API habilitada; cliente OAuth de tipo "Aplicación web" (el de "App de escritorio" no servía, daba `redirect_uri_mismatch` con el OAuth Playground — se borró); refresh token obtenido vía OAuth Playground y probado creando y borrando un evento real en el calendario de Nuria. Las 6 credenciales (Zoom + Google) están en `.env.local`. Se crearon también 5 leads de prueba (4 con datos ficticios + 1 con email real de Nuria) vía el endpoint de ingesta para verificar la bandeja y, pendiente, el envío real. Siguiente: construir la detección de respuesta (P-15) y las dos integraciones en código — no empezado todavía | Nuria + Claude |
