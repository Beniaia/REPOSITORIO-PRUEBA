...
> Última actualización: **25 agosto 2026** · Fase actual: **F0 completa. F1 en marcha: flujo de reunión automática (Zoom + Calendar) — formulario añadido a la ficha del lead, `build`/`typecheck` en verde. Prueba real de punta a punta empezada con el lead ficticio `nuritina10@hotmail.com` pero sin terminar: bloqueada por un problema de login con la contraseña de `nbrotonscongost@gmail.com` en producción, reseteo por email en curso. Sigue pendiente P-15 (detección automática de respuesta) y lanzar la primera tanda real del motor (10 leads de Levante)**
...
| Flujo de reunión automática (Zoom + Calendar) | 🟡 **25/08/2026: formulario añadido en `app/(app)/leads/[id]/page.tsx`** (registrar respuesta del lead + confirmar fecha), `npm run typecheck` y `npm run build` en verde (ambos muy lentos en este Mac por I/O de disco, ~8 y ~35 min — no es un problema del código, ver Riesgos §5). **Prueba real de punta a punta en curso, sin terminar**: bloqueada porque el login de producción no aceptaba la contraseña de `nbrotonscongost@gmail.com` — reseteo por email de recuperación iniciado desde el panel de Supabase, pendiente de que Nuria complete el login y termine la prueba (registrar respuesta → confirmar fecha → comprobar Zoom/Calendar/email reales). Detección automática de respuesta (P-15) sigue sin construir |
...
| Zoom (API, OAuth/Server-to-Server) | ✅ App "Baladre Reuniones" creada, activada y probada el 24/08/2026 (token obtenido con scope `meeting:write:meeting:admin`) | Gratis (cuenta básica de Zoom) |
| Google Cloud / Calendar API | ✅ Proyecto `baladre-prospeccion-piloto`, cliente OAuth web, refresh token obtenido y probado el 24/08/2026 (creación y borrado de un evento real) | Gratis dentro de cuota |
...
9. **Para que el flujo de reunión sea usable de verdad:**
   - a) ~~Añadir el formulario en `app/(app)/leads/[id]/page.tsx`~~ ✅ hecho el 25/08/2026: registrar respuesta del lead + confirmar fecha → dispara `confirmarFechaReunion`.
   - b) ~~`npm run build` y `npm run typecheck` en verde~~ ✅ comprobado el 25/08/2026, sin errores (muy lento por I/O de disco, no por el código, ver Riesgos §5).
   - c) **Probar de verdad — EMPEZADO Y SIN TERMINAR el 25/08/2026.** Bloqueado por login: la contraseña de `nbrotonscongost@gmail.com` en producción no era aceptada; Nuria ha iniciado el reseteo por email de recuperación desde el panel de Supabase. **Siguiente sesión: entrar con la contraseña nueva y completar la prueba paso a paso desde la ficha del lead `nuritina10@hotmail.com`** (permiso de llamada → aprobar → Contactar → registrar respuesta → confirmar fecha) y comprobar que llegan el email, el Zoom y el evento de Calendar.
